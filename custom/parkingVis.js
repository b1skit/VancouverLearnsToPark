...
// Initialize the map, and center it on Vancouver:
...
L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}', {
    ...
    zoomControl	: true,
    id			: 'mapbox.streets',
    accessToken	: 'pk.eyJ1IjoiYjFza2l0IiwiYSI6ImNrMG9ubHd4bjBjcHAzbWp3d2NwdWdoNDYifQ.gopNAShvK6AlH1_qJaWo5w' // NOTE: I signed up for this access token
    }).addTo(theMap);
...
const BYLAW_COLORS =	// TODO: Swap purple and green??? Will need to change icons as well...
{
...
const SECTION_COLORS =	// TODO: MAKE THESE GOOD!!!!!!!!
[
...
const PERIOD_CHART_BAR_COLORS = // City of Vancouver logo colors // TODO: Choose something better!!!
[
    'rgba(1, 126, 197, 0.4)',
    'rgba(137, 200, 63, 0.4)',
...
// Main "Visualize" button functionality:
...
function AddSectionLegend(bylawNumber, infractionText)
{
    ...
    legend.onAdd = function(theMap) 
    {
        ...
        {
            div.innerHTML += '<i style="background: ' + SECTION_BORDER_COLORS[ colorIndex % SECTION_COLORS.length ] + '"></i><span>' + currentText + ": " + infractionText[currentText] + '</span><br>';	 // Wrap the index for the section colors TODO: Choose more colors!
...
function DrawAllLayers()
{
    ...
    _selectedWeekdays = 
    {
        0 : document.getElementById("monCheckbox").checked,
        1 : document.getElementById("tueCheckbox").checked,
        2 : document.getElementById("wedCheckbox").checked,
        3 : document.getElementById("thuCheckbox").checked,
        4 : document.getElementById("friCheckbox").checked,
        5 : document.getElementById("satCheckbox").checked,
        6 : document.getElementById("sunCheckbox").checked,
    };
...
{
    console.log("OnOverlayAddHandler called!!");
}
...
{
    console.log("OnOverlayRemoveHandler called!!");

    // console.log(overlay);

    // for (var curr in overlayObjects)
    // {
    // 	if (overlay == overlayObjects[curr])
    // 		console.log("gotcha!");
    // }

    // for (var curr in overlayObjects)
    // 	console.log(overlayObjects[curr]);
}
            ...
            [
                {
                    ...
                    data			: 
                    [	
                        assembledResults.stats.weekdayBylawTotals[0][2849], 
    ...
    for (var currentBlock in assembledResults.streetResults[street].block)
    {
        for (var currentInfraction = 0; currentInfraction < assembledResults.streetResults[street].block[currentBlock].infractions.length; currentInfraction++)
        {
            if (assembledResults.streetResults[street].block[currentBlock].infractions[currentInfraction].bylaw == bylaw)
            {
                ...
                var weekday 		= assembledResults.streetResults[street].block[currentBlock].infractions[currentInfraction].weekday;
...
function DrawStreetsByBylawColor()
{
    ...
    for (var currentStreet in assembledResults.streetResults)
    {
        ...
        for (var currentBlock in assembledResults.streetResults[currentStreet].block)
        {
            ...
            {
                // TODO: Do we even need to draw a line if it's going to be transparent??? Leaving this here for now, incase we want to color the lines somehow...
                for (currentSegment = 0; currentSegment < assembledResults.streetResults[currentStreet].block[currentBlock].latLngs.length; currentSegment++)
                {
...
function BlockChartOnClick(clickEvent)
{
    ...
    {
        // console.log("BlockChartOnClick failed at getting valid event elements");
        return;	// 
    }
    else
    {
...
function DrawHeatMap()
{
    ...
    var heatmapOptions =
    {
        ...
        // gradient 	: { 0.1 : 'yellow',  0.5 : 'orange', 0.9 : 'red' },
        // gradient 	: { 0.2 : 'green',  0.65 : 'orange', 0.8 : 'red' },
        // gradient 	: { 0.2 : '#e5f5e0',  0.5 : '#a1d99b', 0.8 : '#31a354' }, // Green hues
        // gradient 	: { 0.2 : '#deebf7',  0.5 : '#9ecae1', 0.8 : '#3182bd' }, // Blue hues
        // gradient 	: { 0.2 : '#fee6ce',  0.5 : '#fdae6b', 0.8 : '#e6550d' }, // Warm hues
...
function GetTotalValidInfractionCount()
{
    ...
    // console.log("Calculated a total of " + infractionCount + " valid infractions");
...
function GetMaxValidBlockInfractions()
{
    ...
    // console.log("Calculated a valid block max of " + max + " infractions");
...
// // Get a ticket status inclusion/exclusion string that can be appended to a database query
// function Get_selectedStatusSearchString()
// {
// 	// Status string codes:
...
// 	var statusString = "&refine.status=";

// 	// Append selected ticket status's here

// 	// return statusString;

// 	return "";	// TODO: Implement this funciton beyond a stub!
// }
...
function CleanseStreetNameString(currentStreetName)
{
    ...
    if (currentStreetName.match(/^(N\s)/))
    {
        ...
        // console.log("Now = " + currentStreetName);
    }
...
function GetSimpleStreetName(currentStreetName)
{
    ...
    // {
    // 	simpleName += " " + nameParts[2];
    // 	console.log("TADA: " + simpleName);
    // }
    // ^^^ Doesn't work...
    if (nameParts.length > 3)
    {
...
function GetPrettyDateString()
{
    ...
    firstDate 		= moment(firstDate).format('DD MMM YYYY');
    ...
    if (parseInt(dateParts[0]) != parseInt(dateParts[3]) || parseInt(dateParts[1]) != parseInt(dateParts[4]) || parseInt(dateParts[2]) != parseInt(dateParts[5]))
    {
        ...
        secondDate = " - " + moment(secondDate).format('DD MMM YYYY');
    }
...
async function AssembleResultsDict(targetStreet)
{
    ...
    for (i = 0; i < streetCenters.records.length; i++)
    {
        ...
        // TODO: Maybe cache of the actual street name??? -> Need it for my popups
        ...
        // if (!streetNames.includes(currentStreetName))
        // {
        // 	streetNames.push(currentStreetName);
        // }
        ...
        {				
            ...
            // console.log("Created new block "+ currentStreetName + " " + currentBlock);

            // // BUG HERE: WE'RE ONLY ADDING A SINGLE ENTRY, SKIPPING 2ND ENTRY WHEN THERE ARE 2+ ENTRIES IN THE STREET CENTERLINES
            // // but, if we move it outside this block, we create many dupes...
        ...
        else
        {
            ...
            for (currentLatLng = 0; currentLatLng < latLngs.length; currentLatLng++) 
            {
                ...
                for (existingLatLngArr = 0; existingLatLngArr < assembledResults.streetResults[currentStreetName].block[currentBlock].latLngs.length; existingLatLngArr++)
                {
                    for (existingLatLng = 0; existingLatLng < assembledResults.streetResults[currentStreetName].block[currentBlock].latLngs[existingLatLngArr].length; existingLatLng++)
                    {
                        ...
                        )
                        {
                            ...
                            // matchingLatLngs = true;
                            // console.log("INNER: found dupe" + currentStreetName + " " + currentBlock);
                            break;
                        }
                    }
                    if (matchingLatLngs[currentLatLng] == true)
                    {
                        // console.log("OUTER: found dupe" + currentStreetName + " " + currentBlock);
                        break;
                    }
                }
                // HACK
                ...
                {
                    // testQuit = true;
                    // console.log("inserting unique " + currentStreetName + " " + currentBlock);
                    // console.log("length b4: " + assembledResults.streetResults[currentStreetName].block[currentBlock].latLngs.length);
                    assembledResults.streetResults[currentStreetName].block[currentBlock].latLngs.push(latLngs); // Push as nested arrays, to handle edge cases where streets aren't divided by block # (eg. bridges, which only have 1 "block")
            ...
            // if (!testQuit)
            // {
            // 	// Can I just insert as soon as I find a unique one?? Or do I need to do this check???
            // 	var hasUniqueLatLng = false;
            // 	for (compareResult = 0; compareResult < matchingLatLngs.length; compareResult++)
            // 	{
            // 		if (matchingLatLngs[compareResult] == false)
            // 		{
            // 			hasUniqueLatLng = true;
            // 			break;
            // 		}
            // 	}
            // 	if (hasUniqueLatLng)
            // 	{
            // 		console.log("inserting uniques!");
            // 		assembledResults.streetResults[currentStreetName].block[currentBlock].latLngs.push(latLngs); // Push as nested arrays, to handle edge cases where streets aren't divided by block # (eg. bridges, which only have 1 "block")
            // 	}
            // }
        ...
        // TODO: Finish additive loading functionality
        // Currently, the latLng arrays AND infraction arrays seem to get duplicate results, for multi-block entries (eg. 900 broughton)
        // -> Move on: Disable this for now...
        // -> Finish remainder of UI functionality, visualizations
        // -> UI Styling/theming
        // -> Implement this last, time permitting
    ...
    for (i = 0; i < tickeResults.records.length; i++)
    {
        ...
        // else
        // {
        // 	console.log("Could not match street " + streetNameKey + "!!!!!!!!!!!!!!!!!!!");

        // 	for (var name in assembledResults.streetResults)
        // 	{
        // 		var testName = streetNameKey.split(" ")[0];
                
        // 		// console.log("testing "+ streetNameKey + " -> " + testName + " vs " + name);
        // 		if (name.includes(testName))
        // 		{
        // 			console.log("Found possible match: " + testName + " -> " + name);
        // 		}
        // 	}
        // } 
    }
    console.log("Could not assign " + debugMissedStreetEntries + " street entries, and " + debugMissedBlockEntries + " block entries to the visible streets");
...
function ComputeAssembledResultsStats()
{
    ...
    // for (var currentStat in assembledResults.stats)
    // {
    // 	for (var currentElement in assembledResults.stats[currentStat])
    // 	{
    // 		assembledResults.stats[currentStat][currentElement] = 0;
    // 	}
    // }
    // // Dirty hack: The above loop stomped our nested stats... Manually recreate them here as a quick fix
    // assembledResults.stats.maximums.block 	= { count : 0, streetName : "UNINITIALIZED_DATA", blockNumber : -1};
    // assembledResults.stats.maximums.street 	= { count : 0, streetName : "UNINITIALIZED_DATA" };
    ...
    for (var currentStreet in assembledResults.streetResults)
    {
        ...
        for (var currentBlock in assembledResults.streetResults[currentStreet].block)
        {
            ...
            for (var currentInfraction = 0; currentInfraction < assembledResults.streetResults[currentStreet].block[currentBlock].infractions.length; currentInfraction++)
            {
                ...
                // assembledResults.stats.weekdayBylawTotals[parseInt(currentWeekday)][parseInt(currentBylaw)]++;
                assembledResults.stats.weekdayBylawTotals[currentWeekday][currentBylaw]++;
                

                // assembledResults.stats.weekdayTotals[assembledResults.streetResults[currentStreet].block[currentBlock].infractions[currentInfraction].weekday]++;
                // assembledResults.stats.bylawTotals[assembledResults.streetResults[currentStreet].block[currentBlock].infractions[currentInfraction].bylaw]++;
                // assembledResults.stats.statusTotals[assembledResults.streetResults[currentStreet].block[currentBlock].infractions[currentInfraction].status]++;
            }

            // Per-bylaw, per-infraction stats:
...
async function QueryParkingTicketDB(streetNames, DBURL)
{
    ...
    // BUG HERE: We're requiring an EXACT match (ie to ensure main != mainland), but this is excluding W 4th Av / 4th Av W
    // Maybe cleanse more?
    // Maybe use a single word as the key (instead of entire street name), and cache the "real" street name off as a member string??
    ...
    {
        // var bylawString 	= "&refine.bylaw=2849&refine.bylaw=2952&refine.bylaw=9344&refine.bylaw=9978&disjunctive.bylaw=true";
